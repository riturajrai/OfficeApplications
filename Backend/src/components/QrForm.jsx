...
export default function QRForm() {
    ...
    const fetchFormData = async () => {
      try {
        ...
        setFormData((prev) => ({
          ...
          designation: response.data.designations[0]?.name || '',
          department_name: response.data.departments[0]?.name || '',
        }));
  ...
  const isFormValid = () => {
    const { name, email, application_type, designation } = formData;
    const emailValid = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email);

    return (
      name.trim() &&
      emailValid &&
      application_type.trim() &&
      designation.trim() &&
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      form.append('department_name', formData.department_name);
      ...
      <div className="max-w-lg w-full bg-white dark:bg-gray-800 rounded-lg shadow-md border border-gray-200 dark:border-gray-700 overflow-hidden">
        ...
        <div className="p-4 sm:p-6 space-y-5">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1">
              <label className="flex items-center text-[12px] font-medium text-gray-700 dark:text-gray-300">
                <BriefcaseIcon className="w-4 h-4 mr-1.5 text-gray-500 dark:text-gray-400" />
                Designation <span className="text-pink-500 ml-1">*</span>
              </label>
              <select
                name="designation"
            ...
            <div className="space-y-1">
              <label className="flex items-center text-[12px] font-medium text-gray-700 dark:text-gray-300">
                <BuildingOffice2Icon className="w-4 h-4 mr-1.5 text-gray-500 dark:text-gray-400" />
                Department
              </label>
              <select
